...
public class GalleryAdapter extends RecyclerView.Adapter<GalleryAdapter.ViewHolder> {
    ...
    public List<MediaItem> getSelectedItems() {
        ...
        for (int pos : selectedPositions) if (pos < items.size()) selected.add(items.get(pos));
        return selected;
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder h, int position) {
        ...
        int badgeIcon;
        switch (item.getType()) {
            case VIDEO: badgeIcon = R.drawable.ic_video; break;
            case AUDIO: badgeIcon = R.drawable.ic_audio; break;
            case IMAGE: badgeIcon = R.drawable.ic_image; break;
            default:    badgeIcon = R.drawable.ic_file; break;
        }
        h.ivTypeBadge.setImageResource(badgeIcon);

        // Thumbnail
        if (item.getType() == MediaItem.Type.IMAGE) {
            Glide.with(context).load(new File(item.getFilePath()))
                    .centerCrop().placeholder(R.drawable.ic_image).into(h.ivThumb);
        } else if (item.getType() == MediaItem.Type.VIDEO) {
            Glide.with(context).load(new File(item.getFilePath()))
                    .centerCrop().placeholder(R.drawable.ic_video).into(h.ivThumb);
        } else if (item.getType() == MediaItem.Type.AUDIO) {
            h.ivThumb.setImageResource(R.drawable.ic_audio);
            h.ivThumb.setScaleType(ImageView.ScaleType.CENTER);
        } else {
    ...
    static class ViewHolder extends RecyclerView.ViewHolder {
        ImageView ivThumb, ivTypeBadge;
        TextView tvName, tvSize;
        CheckBox cbSelect;
        ViewHolder(View v) {
            super(v);
            ivThumb    = v.findViewById(R.id.iv_thumb);
            ivTypeBadge= v.findViewById(R.id.iv_type_badge);
